...
export const CONTRACT_ADDRESS =
  "CBF4SQHZ6N4M2C427BY26U4ENXRX3MVRAMBKNTWTAWYRWEESJJAZWNIY";
 ...
 * Calls: register_vehicle(vin: String, owner: String, make: String, model: String, year: u32)
 */
...
/**
 * Add history record to a vehicle - PERMISSIONLESS, anyone can add.
 * Calls: add_history(vin: String, record: String, record_type: String)
 * record_type can be: "service", "accident", "ownership", "inspection", "recall"
 */
export async function addHistory(
  ...
  return callContract(
    "add_history",
    [toScValString(vin), toScValString(record), toScValString(recordType)],
    caller,
    true
  );
}
 ...
 * Calls: get_vehicle(vin: String) -> Vehicle
 * Returns: { vin, owner, make, model, year, history: Vec<HistoryRecord> }
 */
export async function getVehicle(vin: string, caller?: string) {
  return readContract("get_vehicle", [toScValString(vin)], caller);
}

/**
 * Get vehicle history - PERMISSIONLESS, anyone can view.
 * Calls: get_history(vin: String) -> Vec<HistoryRecord>
 * Returns: Array of { record: string, record_type: string }
 */
export async function getHistory(vin: string, caller?: string) {
  return readContract("get_history", [toScValString(vin)], caller);
}

/**
 * Get all registered VINs - PERMISSIONLESS, anyone can view.
 * Calls: get_all_vins() -> Vec<String>
 * Returns: Array of VIN strings
 */
export async function getAllVins(caller?: string) {
  return readContract("get_all_vins", [], caller);
}
